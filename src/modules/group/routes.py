...
@router.get("/{scheduleId}/level-schedules/")
async def get_groups_same_level_subject(scheduleId: int, pensumIds: list[int] = Query(...), groupId: int = Query()):
    try:
        schedule_pensum_ids = []
        for pensumId in pensumIds:
            schedule_pensum = await get_schedule_pensum_by_pensum_id_and_schedule_id(pensumId, scheduleId)
...
@router.put("/mark-as-mirror-any/", status_code=status.HTTP_200_OK)
async def mark_as_mirror_any(group_ids: list[int]):
    try:
        return await update_mirror_group_any(group_ids)
    except Exception as e:
        raise HTTPException(status_code=422, detail=str(e))