...
async def get_groups_by_pensum(
    ...
    try:
        schedule_pensums = await database.academic_schedule_pensum.find_many(
            where={"academicScheduleId": schedule_id, "pensumId": {"in": pensumIds}}
        )
        schedule_pensums_id = [sp.id for sp in schedule_pensums]
        if not schedule_pensums_id:
            return {"total": 0, "skip": skip, "take": take, "data": [], "has_more": False}

        result = await get_all_groups_by_schedule_pensum_id(schedule_pensums_id, skip=skip, take=take)
        has_more = (result["skip"] + len(result["data"])) < result["total"]
...
async def create_group(request: GroupCreationRequest):
    ...
    try:
        ...
        academic_schedule_pensum = await get_schedule_pensum_by_pensum_id_and_schedule_id(pensum_id, academic_schedule_id)
...
@router.get("/schedule/{academicScheduleId}/all", status_code=status.HTTP_200_OK, response_model=List[GroupResponse])
