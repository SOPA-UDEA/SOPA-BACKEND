...
from src.modules.group_classroom.services import get_group_classroom, get_classroom_and_schedules
...
async def upload_classroom_x_group(file: BinaryIO):
    ...
    for index, row in df.iterrows():
        ...
        group_data = await get_group_by_code_and_subject_code(code=int(row['GR']), subject_code=subject_code)
        ...
        for i, classroom in enumerate(classroom_list):
            ...
            if classroom_data is None:
                ...
                new_classroom = ClassroomRequest(
                    capacity= None if classroom in ('INGENIA', 'UDE@') else int(row['CUPO']),
            ...
            if i >= len(schedules_blocks):
                raise HTTPException(status_code=400, detail=f"Error: Not enough schedules for classroom {classroom}")
            ...
            for schedule in individual_schedule:
                ...
                if classroom not in ('INGENIA', 'UDE@'):
                    ...
                    classrooms_groups = await get_classroom_and_schedules(classroomId=classroom_data.id, days=days)
                    if len(classrooms_groups) > 0:
                        #check if there is a conflict with the existing schedules
                        if has_conflict([cg.schedule for cg in classrooms_groups], schedule):
                            raise HTTPException(status_code=400, detail=f"Error: Conflict with existing schedules for classroom {classroom} and schedule {schedule}")
                
                #create classroom_x_group
                print(f"Creating classroom x group for classroom {classroom} and group {group_data.code}")
